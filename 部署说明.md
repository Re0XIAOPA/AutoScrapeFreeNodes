...
如果你发现每次GitHub Actions构建后自定义域名失效，有两种解决方法：

**方法一: 使用改进版的generate-static.js**

此仓库已更新`generate-static.js`脚本，能够正确保留自定义域名。确保你的`CNAME`文件中直接包含域名（不带注释）。例如：

```
yourdomain.com
```

**方法二: 修改GitHub Actions工作流**

此仓库的`.github/workflows/deploy.yml`文件已经添加了以下配置，防止CNAME文件被清除：
...
此配置确保即使是全新构建的网站也会保留gh-pages分支上现有的CNAME文件。
...
这表示Git无法识别当前分支应该推送到哪个远程分支。解决方法：
...
2. **Fork后的仓库分支名不一致**：
   - 有时候Fork的仓库默认分支名可能是`master`而不是`main`
   - 在这种情况下，需要修改工作流文件中的分支名称，或者在GitHub中将默认分支重命名为`main`

3. **更改工作流检出设置**：
   - 在工作流的checkout步骤中添加`fetch-depth: 0`参数可以获取完整的提交历史，有助于解决某些分支追踪问题
